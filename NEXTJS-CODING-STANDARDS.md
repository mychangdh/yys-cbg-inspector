...
- 页面或复用组件的样式、类型文件按项目既有三件套约定放在同目录：**index.tsx**、**index.scss**、**index.types.ts**。
...
- 页面、组件 Props、事件和回调类型放入同目录类型文件；跨页面领域类型放入 **src/types/**。
...
- 每次交付至少运行 **npm run test** 和 **npm run build**；生产构建不得忽略 TypeScript 错误。

## 7. 样式与资源规范

- 组件样式与组件同目录同名，使用 SCSS 的嵌套、变量、混入等能力维护，不把组件样式堆到全局文件。
...
- 一个 **.tsx** 文件只承载一个主要 React 组件；第二个可复用单元必须拆分并提供自己的样式与类型文件。
