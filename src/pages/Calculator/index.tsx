...
export function CalculatorWorkspace() {
  ...
  const suitTypes = useMemo(() => {
    ...
    return (config.yuhun_list || [])
      .map(([id, name, , attributeOrOnePiece = "", effectOrTwoPiece = ""]) => ({
        id: Number(id),
        name: String(name),
        twoPieceText: omaIds.has(Number(id))
          ? String(effectOrTwoPiece).trim() || String(attributeOrOnePiece)
          : String(attributeOrOnePiece),
  ...
  const run = () => {
    if (running) return;
    if (!hero) {
      void message.warning("请先选择式神后再开始计算");
      return;
    }
    if (!hasCompleteMainAttributeSelection) {
      void message.warning("请为 2 / 4 / 6 号位各选择至少一种主属性");
      return;
    }
