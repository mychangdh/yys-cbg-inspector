...
function countCollectionSkins(
  detail: { skin?: { ss?: unknown } },
  gameConfig: GameConfig,
) {
  const collectionSkinIds = new Set(
    Object.keys(gameConfig.collection_skin_data || {}),
  );
  const skins = detail.skin?.ss;
  if (!Array.isArray(skins)) return 0;
  return skins.filter(
    (skin: unknown) =>
      Array.isArray(skin) && collectionSkinIds.has(String(skin[2])),
  ).length;
}

/** 从商品原始 skin.ss 按静态典藏皮肤 ID 精确统计典藏数量。 */
export function extractCbgCollectionSkinCount(
  payload: unknown,
  gameConfig: GameConfig,
) {
  const source = payload as {
    equip?: { equip_desc?: unknown };
    equip_data?: { equip_desc?: unknown };
  };
  const equip = source.equip || source.equip_data;
  if (!equip?.equip_desc) return undefined;
  const detail =
    typeof equip.equip_desc === "string"
      ? JSON.parse(equip.equip_desc)
      : equip.equip_desc;
  return countCollectionSkins(detail, gameConfig);
}
