...
import { CalculatorRunningState } from "../CalculatorRunningState";
...
import { relicPanelCeiling } from "../CalculatorRangeField";
import { CalculatorMethodInfo } from "../CalculatorMethodInfo";
...
export function CalculatorWorkspace() {
  ...
  const {
    ...
    startCalculation,
    stopCalculation,
  } = useRelicCalculation(messageApi);
  ...
  const openRelicConfigEditor = (result: CalculatorResult) => {
    ...
    if (relicIds.size !== result.relics.length) {
      void messageApi.warning("当前御魂组合缺少御魂编号，无法保存");
      return;
    }
  ...
  const saveRelics = () => {
    ...
    if (savedRelics.some((relic) => getSavedRelicSources(relic).includes(label))) {
      void messageApi.warning("御魂方案名称已存在，请重新修改名称");
      return;
    }
    const relicIds = new Set(
    ...
    void updateRelics(relicIds, (relic) => ({
      ...
      .then((matchedCount) => {
        ...
        messageApi.success("御魂已保存");
      })
      .catch(() =>
        messageApi.warning("御魂已保存到当前页面，但本地缓存更新失败"),
      );
  ...
  const applyExcludedRelicPicker = () => {
    ...
    void saveRecentDatasetSnapshot(
      nextDataset,
      nextDataset.account?.sourceUrl || "",
    ).catch(() => messageApi.warning("排除设置已更新，但本地缓存更新失败"));
  ...
  const removeSavedRelicGroup = (source: string) => {
    ...
    void saveRecentDatasetSnapshot(
      nextDataset,
      nextDataset.account?.sourceUrl || "",
    ).catch(() => messageApi.warning("分组已删除，但本地缓存更新失败"));
  };
