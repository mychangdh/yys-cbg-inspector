  ...
  const startCalculation = useCallback(
    (request: CalculationRequest) => {
      ...
      window.setTimeout(() => {
        ...
        // 同一个 Worker 内部会先计算明确两件套，再计算散件覆盖路径，并
        // 复用候选缓存和首阶段最优值。此前把两条路径拆成两轮 Worker，会
        // 将完整御魂仓库结构化克隆两次、JIT 预热两次；在大仓库中这部分
        // 开销超过跨 Worker 共享下界带来的剪枝收益，极速反而可能更慢。
        // 每个布局只归属于一个 Worker，其局部最优可安全作为该 Worker
        // 后续搜索的下界，最终仍统一合并所有布局的最优结果。
