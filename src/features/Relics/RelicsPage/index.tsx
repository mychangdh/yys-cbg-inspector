...
type RelicSortDirection = "asc" | "desc";
    ...
    <div className={`width result ${styles.page}`}>
      ...
      <Card className={styles.panel}>
        <div className={styles.filters}>
          <Button
            className={`${styles.filter} ${styles.suitFilter}`}
          ...
          {variableMainPositions.has(Number(activePosition)) ? (
            ...
            <div className={styles.fixedMainAttribute} aria-label="固定主属性">
              <span>主属性</span>
              <strong>
          ...
          <Select
            ...
            className={`${styles.filter} ${styles.subFilter}`}
          ...
          <Select
            allowClear
            showSearch={false}
            className={`${styles.filter} ${styles.sortFilter}`}
            placeholder="副属性排序"
            optionFilterProp="label"
            value={sortAttribute}
        ...
        <Tabs
          ...
          items={[1, 2, 3, 4, 5, 6].map((position) => ({
            key: String(position),
            label: (
              <span className={styles.positionTabLabel}>
                <span>{position}号位</span>
                <span className={styles.positionTabCount}>
