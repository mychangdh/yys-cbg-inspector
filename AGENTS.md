...
- 每个可复用单元必须拥有同目录、同名的样式文件和类型文件；不同语言按项目约定使用对应扩展名。
- 单元的输入、输出、状态、配置、事件、回调及数据结构类型优先放入对应类型文件；跨页面复用的领域类型才放入公共类型目录。实现文件只导入和使用类型文件，不重复声明同一套类型。
...
- 不同运行端保持相同的页面/组件文件组织方式；新增页面或可复用单元时，各端同步创建对应样式文件和类型文件。
...
- 每个实现文件必须实际导入并使用对应的样式与类型文件；只创建空文件、未接入文件或仅为通过检查的类型别名不视为完成。
- 一个实现文件只承载一个主要单元；如果文件中出现第二个可复用单元，应拆到自己的目录，并同步提供样式文件和类型文件。

### 14.2 React / TypeScript 专项约定

- 一个 `.tsx` 文件只能定义一个 React 组件；组件 Props、状态、事件和回调类型放在同目录的 `.types.ts` 中，并由 `.tsx` 显式导入使用。
...
- React 页面允许在页面 `.tsx` 中编排子组件，但子组件必须拥有自己的 `.tsx`、`.scss` 和 `.types.ts` 文件；跨组件复用的类型迁移到 `src/types/`。
...
- 组件目录和具体页面目录（例如 `src/components/Layout`、`src/pages/Home`）统一使用大驼峰命名（PascalCase）；`src/pages` 容器目录、路由目录、store 目录及其他普通目录统一使用小驼峰命名（camelCase）。页面或组件目录内的入口文件统一命名为 `index`：组件/页面按需提供 `index.tsx`、`index.scss`、`index.types.ts`，公共目录也使用 `index` 作为聚合入口。
